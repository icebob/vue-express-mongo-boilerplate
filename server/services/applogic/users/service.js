...
module.exports = {
	...
	actions: {
		...
		create: {
			...
			handler(ctx) {
				return this.Promise.resolve(ctx)
				// .then(() => crypto.randomBytes(25, function(err, buf) {
				// 	return err ? null : buf.toString("hex");
				// }))
				// .then(() => crypto.randomBytes(25).toString("hex"))
				// .then(resetToken => {
				.then(() => {
				...
				.then(() => mailer.send(ctx.params.email, "mailSubjectResetPassword", "please reset " + ctx.params.fullName + " at the link "));	
			}
		},
